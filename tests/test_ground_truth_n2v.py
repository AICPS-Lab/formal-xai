#!/usr/bin/env python3
"""Ground truth test: n2v backend vs n2v oracle.

Uses the committed test model (tests/models/) and n2v oracle ground truth
(tests/ground_truth/n2v/) to verify that the n2v backend produces
...
GROUND_TRUTH_DIR = os.path.join(TESTS_DIR, "ground_truth", "n2v")

EPSILON = 25 / 255
NUM_CLASSES = 10


def test_n2v_ground_truth():
    """Run VitaX + n2v on the test model and compare to saved oracle."""
    ...
    for f in sorted(os.listdir(GROUND_TRUTH_DIR)):
        ...
        # Strict checks — must be bit-identical (same backend, same model, same seed)
    ...
    verifier.close()
    print(f"{'='*50}")
    print(f"n2v Ground Truth: {passed} passed, {failed} failed")
    print(f"{'='*50}")
    return failed == 0
...
if __name__ == "__main__":
    print("=" * 60)
    print("VitaX Ground Truth Test — n2v backend")
    print("=" * 60)
    ...
    if not os.path.isdir(GROUND_TRUTH_DIR):
        ...
        print("Run: python tests/scripts/generate_oracle.py --backend n2v")
        sys.exit(1)

    ok = test_n2v_ground_truth()
